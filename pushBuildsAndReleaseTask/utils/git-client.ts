...
export class GitClient {
  ...
  configUsernameG(username: string): void {
    this.spawnGitProcess(['config', '--global', 'user.name', username]);
  }

  configUserEmailG(email: string): void {
    this.spawnGitProcess(['config', '--global', 'user.email', email]);
  }
